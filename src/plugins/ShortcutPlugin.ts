...
import { makePluginByCombiningPlugins, makeWrapResolversPlugin } from 'graphile-utils';
...
module.exports = makePluginByCombiningPlugins(
  ...
  function addShortcutArgument(builder): void {
  ...
  function addShortcutField(builder): void {
  ...
  makeWrapResolversPlugin(
    ...
    ({ scope }) => async (resolver, source, args, context, info) => {
      const { pgClient: { processID } } = context;
      ...
      if (shortcutData && typeof shortcutData === 'object') {
        for (const shortcutDataKey of Object.keys(shortcutData)) {
